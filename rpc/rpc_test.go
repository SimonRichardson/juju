...
func (r *Root) InterfaceMethods(id string) (InterfaceMethods, error) {
	logger.Infof(ctx, "interface methods called")
...
func (cc *CustomRoot) FindMethod(
	...
	logger.Debugf(ctx, "got to FindMethod: %q %d %q", rootMethodName, version, objMethodName)
	...
	logger.Debugf(ctx, "found type: %s", goType)
...
func (c *testCodec) WriteMessage(hdr *rpc.Header, x interface{}) error {
	...
	logger.Infof(ctx, "send header: %#v; body: %#v", hdr, x)
...
func (c *testCodec) ReadHeader(hdr *rpc.Header) error {
	...
	logger.Infof(ctx, "got header %#v", hdr)
	if c.role != roleBoth && hdr.IsRequest() == (c.role == roleClient) {
...
func (c *testCodec) ReadBody(r interface{}, isRequest bool) error {
	...
	logger.Infof(ctx, "got response body: %q", m)
	err = json.Unmarshal(m, r)
	logger.Infof(ctx, "unmarshalled into %#v", r)
	return err
}
