...
func (st *State) GetUnitWatchIdentifiers(ctx context.Context, name coreunit.Name) (application.UnitWatchIdentifiers, error) {
	db, err := st.DB(ctx)
	if err != nil {
	...
	ident := unitName{Name: name.String()}
	unitStmt, err := st.Prepare(`
SELECT u.uuid AS &unitWatchIdentifier.unit_uuid,
       u.application_uuid AS &unitWatchIdentifier.application_uuid,
       u.charm_uuid AS &unitWatchIdentifier.charm_uuid
FROM   unit AS u
WHERE  u.name = $unitName.name
`, unitWatchIdentifier{}, ident)
	if err != nil {
		return application.UnitWatchIdentifiers{}, errors.Capture(err)
	}
...
SELECT DISTINCT re.relation_uuid AS &relationWatchIdentifier.relation_uuid,
       ru.uuid AS &relationWatchIdentifier.relation_unit_uuid
...
`, entityUUID{}, ident)
	...
	var unit unitWatchIdentifier
	var netNodes []unitNetNodeUUID
	var relations []relationWatchIdentifier
	...
	result := application.UnitWatchIdentifiers{
	...
	for i, relationEndpoint := range relationEndpoints {
		result.RelationEndpointUUIDs[i] = relationEndpoint.UUID
	}
	return result, nil
}
