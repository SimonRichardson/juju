...
func FindInstanceSpec(possibleImages []Image, ic *InstanceConstraint, allInstanceTypes []InstanceType) (*InstanceSpec, error) {
	logger.Debugf(ctx, "instance constraints %+v", ic)
	...
	logger.Debugf(ctx, "matching constraints %v against possible image metadata %s", ic, pretty.Sprint(possibleImages))
	...
	if len(specs) > 0 {
		...
		logger.Infof(ctx, "find instance - using %v image of type %v with id: %v", specs[0].Image.Arch, specs[0].InstanceType.Name, specs[0].Image.Id)
		return specs[0], nil
	}
