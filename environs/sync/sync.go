...
func SyncTools(stdCtx context.Context, syncContext *SyncContext) error {
	...
	logger.Infof(ctx, "listing available agent binaries")
	...
	logger.Infof(ctx, "found %d agent binaries", len(sourceTools))
	for _, tool := range sourceTools {
		logger.Debugf(ctx, "found source agent binary: %v", tool)
	}

	logger.Infof(ctx, "listing target agent binaries storage")

	result, err := sourceTools.Match(coretools.Filter{Number: syncContext.ChosenVersion})
	logger.Tracef(ctx, "syncContext.ChosenVersion %s, result %s", syncContext.ChosenVersion, result)
	if err != nil {
		return errors.Wrap(err, errors.NotFoundf("%q", syncContext.ChosenVersion))
	}
	_, chosenList := result.Newest()
	logger.Tracef(ctx, "syncContext.ChosenVersion %s, chosenList %s", syncContext.ChosenVersion, chosenList)
	if syncContext.TargetToolsFinder != nil {
		...
		for _, tool := range targetTools {
			logger.Debugf(ctx, "found target agent binary: %v", tool)
		}
	...
	if syncContext.DryRun {
		for _, tools := range chosenList {
			logger.Infof(ctx, "copying %s from %s", tools.Version, tools.URL)
		}
		return nil
	}
	...
	logger.Infof(ctx, "copied %d agent binaries", len(chosenList))
	return nil
}
...
func buildAgentTarball(
	...
	logger.Debugf(ctx, "Making agent binary tarball")
	...
	logger.Infof(ctx, "using %s %v aliased to %v (%dkB)", agentBinary, toolsVersion, forceVersion, (size+512)/1024)
