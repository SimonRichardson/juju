...
func (b *AgentBootstrap) ensureInitialModel(
	...
	if len(stateParams.InitialModelConfig) == 0 {
		b.logger.Debugf(ctx, "no initial model configured")
		return nil
	}
...
func (b *AgentBootstrap) initBootstrapMachine(
	...
	stateParams := b.stateInitializationParams
	b.logger.Infof(ctx, "initialising bootstrap machine with config: %+v", stateParams)
...
func (b *AgentBootstrap) initBootstrapNode(
	...
	b.logger.Debugf(ctx, "initialising bootstrap node for with config: %+v", b.stateInitializationParams)
