...
func NewStateControllerConfig(st ControllerConfigState, cc ControllerConfigGetter) *ControllerConfigAPI {
...
func (s *ControllerConfigAPI) ControllerConfig() (params.ControllerConfigResult, error) {
	...
	config, err := s.cc.ControllerConfig(context.TODO())
	if err != nil {
		return result, err
	}
...
func (s *ControllerConfigAPI) getModelControllerInfo(model params.Entity) (params.ControllerAPIInfoResult, error) {
	...
	if modelExists {
		addrs, caCert, err := StateControllerInfo(s.st, s.cc)
		if err != nil {
			return params.ControllerAPIInfoResult{}, errors.Trace(err)
		}
...
func StateControllerInfo(st controllerInfoState, cc ControllerConfigGetter) (addrs []string, caCert string, _ error) {
	addr, err := apiAddresses(st, cc)
	if err != nil {
		return nil, "", errors.Trace(err)
	}
	controllerConfig, err := cc.ControllerConfig(context.TODO())
	if err != nil {
		return nil, "", errors.Trace(err)
	}
