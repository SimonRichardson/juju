...
type controllerConfigSuite struct {
	...
	mockControllerConfigGetter *mocks.MockControllerConfigGetter
}
...
func (s *controllerConfigSuite) setup(c *gc.C) *gomock.Controller {
	...
	s.mockControllerConfigGetter = mocks.NewMockControllerConfigGetter(ctrl)

	s.st = mocks.NewMockControllerConfigState(ctrl)
	s.cc = common.NewStateControllerConfig(s.st, s.mockControllerConfigGetter)
	return ctrl
}
...
func (s *controllerInfoSuite) TestControllerInfoLocalModel(c *gc.C) {
	cc := common.NewStateControllerConfig(s.localState, s.cc)
