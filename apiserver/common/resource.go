...
func (rs *Resources) Register(r worker.Worker) string {
	...
	logger.Tracef(ctx, "registered unnamed resource: %s", id)
	return id
}
...
func (rs *Resources) RegisterNamed(name string, r worker.Worker) error {
	...
	logger.Tracef(ctx, "registered named resource: %s", name)
	return nil
}
...
func (rs *Resources) Stop(id string) error {
	...
	logger.Tracef(ctx, "stopping resource: %s", id)
	r := rs.Get(id)
	if r == nil {
		return nil
	}
...
func (rs *Resources) StopAll() {
	...
	for i := len(rs.stack); i > 0; i-- {
		...
		if err := r.Wait(); err != nil {
			logger.Errorf(ctx, "error stopping %T resource: %v", r, err)
		}
	}
