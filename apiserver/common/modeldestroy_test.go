...
func (s *destroyModelSuite) TestDestroyModelSendsMetrics(c *gc.C) {
	err := common.DestroyModel(s.modelManager, nil, nil, nil, nil, s.cc)
...
func (s *destroyModelSuite) testDestroyModel(c *gc.C, destroyStorage, force *bool, maxWait, timeout *time.Duration) {
	...
	err := common.DestroyModel(s.modelManager, destroyStorage, force, maxWait, timeout, s.cc)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *destroyModelSuite) TestDestroyModelIgnoresErrorsWithForce(c *gc.C) {
	...
	true_ := true
	err := common.DestroyModel(s.modelManager, nil, &true_, nil, nil, s.cc)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *destroyModelSuite) TestDestroyControllerNonControllerModel(c *gc.C) {
	...
	err := common.DestroyController(s.modelManager, false, nil, nil, nil, nil, s.cc)
...
func (s *destroyModelSuite) TestDestroyControllerReleaseStorage(c *gc.C) {
	...
	err := common.DestroyController(s.modelManager, false, &destroyStorage, nil, nil, nil, s.cc)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *destroyModelSuite) TestDestroyControllerForce(c *gc.C) {
	...
	timeout := time.Hour
	maxWait := time.Second
	err := common.DestroyController(s.modelManager, false, nil, &force, &maxWait, &timeout, s.cc)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *destroyModelSuite) TestDestroyControllerDestroyHostedModels(c *gc.C) {
	err := common.DestroyController(s.modelManager, true, nil, nil, nil, nil, s.cc)
	c.Assert(err, jc.ErrorIsNil)
