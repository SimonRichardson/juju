...
type ToolsURLGetter interface {
	...
	ToolsURLs(v version.Binary, cc ControllerConfigGetter) ([]string, error)
}
...
type ToolsFinder interface {
	FindAgents(args FindAgentsParams, cc ControllerConfigGetter) (coretools.List, error)
}
...
func (f *toolsFinder) FindAgents(args FindAgentsParams, cc ControllerConfigGetter) (coretools.List, error) {
	...
	for _, baseTools := range list {
		urls, err := f.urlGetter.ToolsURLs(baseTools.Version, cc)
		if err != nil {
			return nil, err
		}
...
func (t *toolsURLGetter) ToolsURLs(v version.Binary, cc ControllerConfigGetter) ([]string, error) {
	addrs, err := apiAddresses(t.apiHostPortsGetter, cc)
	if err != nil {
		return nil, err
	}
	if len(addrs) == 0 {
