...
func NewProvisionerAPI(ctx facade.Context) (*ProvisionerAPI, error) {
	...
	api := &ProvisionerAPI{
		Remover:              common.NewRemover(st, ctx.ObjectStoreFactory().ModelObjectStore(), nil, false, getAuthFunc),
	...
	api.toolsFinder = common.NewToolsFinder(serviceFactory.ControllerConfig(), configGetter, st, urlGetter, newEnviron, ctx.ObjectStoreFactory())
