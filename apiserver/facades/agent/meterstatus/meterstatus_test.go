...
func (s *meterStatusSuite) SetUpTest(c *gc.C) {
	...
	status, err := meterstatus.NewMeterStatusAPI(s.State, s.resources, s.authorizer, loggo.GetLogger("juju.apiserver.meterstatus"), s.cc)
...
func (s *meterStatusSuite) setupMeterStatusAPI(c *gc.C, fn func(meterStatusAPIMocks)) (*meterstatus.MeterStatusAPI, *gomock.Controller) {
	...
	status, err := meterstatus.NewMeterStatusAPI(mockState, mockResources, mockAuthorizer, loggo.GetLogger("juju.apiserver.meterstatus"), s.cc)
	c.Assert(err, jc.ErrorIsNil)
