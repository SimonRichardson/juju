...
func (s *upgraderSuite) SetUpTest(c *gc.C) {
	...
	c.Assert(err, jc.ErrorIsNil)
	s.upgrader, err = upgrader.NewUpgraderAPI(systemState, s.State, s.resources, s.authorizer, loggo.GetLogger("juju.apiserver.upgrader"), s.cc)
	c.Assert(err, jc.ErrorIsNil)
}
...
func (s *upgraderSuite) TestWatchAPIVersionApplication(c *gc.C) {
	...
	systemState, err := s.StatePool.SystemState()
	c.Assert(err, jc.ErrorIsNil)
	upgrader, err := upgrader.NewUpgraderAPI(systemState, s.State, s.resources, authorizer, loggo.GetLogger("juju.apiserver.upgrader"), s.cc)
	c.Assert(err, jc.ErrorIsNil)
	args := params.Entities{
...
func (s *upgraderSuite) TestWatchAPIVersionRefusesWrongAgent(c *gc.C) {
	...
	systemState, err := s.StatePool.SystemState()
	c.Assert(err, jc.ErrorIsNil)
	anUpgrader, err := upgrader.NewUpgraderAPI(systemState, s.State, s.resources, anAuthorizer, loggo.GetLogger("juju.apiserver.upgrader"), s.cc)
...
func (s *upgraderSuite) TestDesiredVersionUnrestrictedForAPIAgents(c *gc.C) {
	...
	systemState, err := s.StatePool.SystemState()
	c.Assert(err, jc.ErrorIsNil)
	upgraderAPI, err := upgrader.NewUpgraderAPI(systemState, s.State, s.resources, authorizer, loggo.GetLogger("juju.apiserver.upgrader"), s.cc)
	c.Assert(err, jc.ErrorIsNil)
