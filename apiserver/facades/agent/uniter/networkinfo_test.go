...
func (s *networkInfoSuite) TestProcessAPIRequestForBinding(c *gc.C) {
	...
	err = machine.SetProviderAddresses(s.ControllerConfigAttrs, network.NewSpaceAddress("10.2.3.4/16"))
	c.Assert(err, jc.ErrorIsNil)
...
func (s *networkInfoSuite) TestProcessAPIRequestBridgeWithSameIPOverNIC(c *gc.C) {
	...
	// retrieved up-front to use as a fallback when we fail to locate addresses
	// on link-layer devices.
	err = machine.SetProviderAddresses(s.ControllerConfigAttrs, network.NewSpaceAddress(ip))
	c.Assert(err, jc.ErrorIsNil)
...
func (s *networkInfoSuite) TestAPIRequestForRelationIAASHostNameIngressNoEgress(c *gc.C) {
	...
	err = machine.SetProviderAddresses(s.ControllerConfigAttrs, addr)
	c.Assert(err, jc.ErrorIsNil)
	...
	retryFactory := func() retry.CallArgs {
		...
		return retry.CallArgs{
			...
			NotifyFunc: func(lastError error, attempt int) {
				...
				if attempt == 1 {
					err := machine.SetProviderAddresses(s.ControllerConfigAttrs, network.NewSpaceAddress("4.3.2.1", network.WithScope(network.ScopeCloudLocal)))
					c.Assert(err, jc.ErrorIsNil)
				}
			},
		}
