...
func (m *ModelManagerAPI) newCAASModel(
	...
	controllerConfig, err := m.cc.ControllerConfig(stdcontext.TODO())
	if err != nil {
...
func (m *ModelManagerAPI) newModel(
	...
	controllerCfg, err := m.cc.ControllerConfig(stdcontext.TODO())
	if err != nil {
		return nil, errors.Trace(err)
	}
...
func (m *ModelManagerAPI) DestroyModels(ctx stdcontext.Context, cc ControllerConfigGetter, args params.DestroyModelsParams) (params.ErrorResults, error) {
	results := params.ErrorResults{
		Results: make([]params.ErrorResult, len(args.Models)),
	}

	destroyModel := func(modelUUID string, destroyStorage, force *bool, maxWait *time.Duration, timeout *time.Duration, cc ControllerConfigGetter) error {
		...
		err = errors.Trace(common.DestroyModel(st, destroyStorage, force, maxWait, timeout, cc))
		if err != nil {
			return errors.Trace(err)
		}
	...
	for i, arg := range args.Models {
		...
		if err := destroyModel(tag.Id(), arg.DestroyStorage, arg.Force, arg.MaxWait, arg.Timeout, cc); err != nil {
			results.Results[i].Error = apiservererrors.ServerError(err)
			continue
		}
	}
