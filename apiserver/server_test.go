...
func (s *serverSuite) TestAPIHandlerHasPermissionLogin(c *gc.C) {
	...
	st := s.ControllerModel(c).State()
	handler, _ := apiserver.TestingAPIHandlerWithEntity(c, s.StatePool(), st, u, s.cc)
	defer handler.Kill()
...
func (s *serverSuite) TestAPIHandlerHasPermissionLoginToken(c *gc.C) {
	...
	st := s.ControllerModel(c).State()
	handler, _ := apiserver.TestingAPIHandlerWithToken(c, s.StatePool(), st, token, delegator, s.cc)
	defer handler.Kill()
...
func (s *serverSuite) TestAPIHandlerConnectedModel(c *gc.C) {
	...
	otherState := f.MakeModel(c, nil)
	defer otherState.Close()
	handler, _ := apiserver.TestingAPIHandler(c, s.StatePool(), otherState, s.cc)
	defer handler.Kill()
...
func (s *serverSuite) checkAPIHandlerTeardown(c *gc.C, st *state.State) {
	handler, resources := apiserver.TestingAPIHandler(c, s.StatePool(), st, s.cc)
