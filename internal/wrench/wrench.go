...
func IsActive(category, feature string) bool {
	...
	if err != nil {
		logger.Errorf(ctx, "unable to read wrench data for %s/%s (ignored): %v",
	...
	for lines.Scan() {
		...
		if line == feature {
			logger.Tracef(ctx, "wrench for %s/%s is active", category, feature)
			return true
		}
	}
	if err := lines.Err(); err != nil {
		logger.Errorf(ctx, "error while reading wrench data for %s/%s (ignored): %v",
			category, feature, err)
	}
	return false
}
...
func checkWrenchDir(dirName string) bool {
	...
	if err != nil {
		logger.Tracef(ctx, "couldn't read wrench directory: %v", err)
		return false
	}
	if !isOwnedByJujuUser(dirinfo) {
		logger.Errorf(ctx, "wrench directory has incorrect ownership - wrench "+
...
func checkWrenchFile(category, feature, fileName string) bool {
	...
	if err != nil {
		logger.Tracef(ctx, "no wrench data for %s/%s (ignored): %v",
			category, feature, err)
		return false
	}
	if !isOwnedByJujuUser(fileinfo) {
		logger.Errorf(ctx, "wrench file for %s/%s has incorrect ownership "+
	...
	if runtime.GOOS != "windows" {
		if fileinfo.Mode()&0022 != 0 {
			logger.Errorf(ctx, "wrench file for %s/%s should only be writable by "+
