...
func internalConfigureLXDProxies(
	...
	if !running {
		logger.Debugf(ctx, "LXD is not running; skipping proxy configuration")
		return nil
	}
...
func ensureDependencies(lxdSnapChannel string, base corebase.Base) error {
	...
	if lxdViaSnap() {
		...
		if strings.HasPrefix(trackedChannel, lxdSnapChannel) {
			logger.Infof(ctx, "LXD snap is already installed (channel: %s); skipping package installation", trackedChannel)
			return nil
		}

		// We need to switch to a different channel
		logger.Infof(ctx, "switching LXD snap channel from %s to %s", trackedChannel, lxdSnapChannel)
