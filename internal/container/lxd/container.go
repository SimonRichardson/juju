...
func (c *ContainerSpec) ApplyConstraints(serverVersion string, cons constraints.Value) {
	...
	if cons.HasVirtType() {
		...
		if err != nil {
			logger.Errorf(ctx, "failed to parse virt-type constraint %q, ignoring err: %v", *cons.VirtType, err)
		} else {
			c.VirtType = virtType
		}
	}
...
func (c *Container) CPUs() uint {
	...
	if v := c.Config["limits.cpu"]; v != "" {
		...
		if err != nil {
			logger.Errorf(ctx, "failed to parse %q into uint, ignoring err: %s", v, err)
		}
	}
	return cores
}
...
func (c *Container) Mem() uint {
	...
	if err != nil {
		logger.Errorf(ctx, "failed to parse %q into bytes, ignoring err: %s", v, err)
		return 0
	}
	...
	if mib > math.MaxUint32 {
		logger.Errorf(ctx, "byte string %q overflowed uint32, using max value", v)
	...
	for netName, net := range networks {
		...
		for _, addr := range net.Addresses {
			...
			if netAddr.Scope == corenetwork.ScopeLinkLocal || netAddr.Scope == corenetwork.ScopeMachineLocal {
				logger.Tracef(ctx, "ignoring address %q for container %q", addr, name)
				continue
			}
...
func (s *Server) CreateContainerFromSpec(spec ContainerSpec) (*Container, error) {
	logger.Infof(ctx, "starting new container %q (image %q)", spec.Name, spec.Image.Image.Filename)
	logger.Debugf(ctx, "new container has profiles %v", spec.Profiles)
	...
	logger.Debugf(ctx, "created container %q, waiting for start...", spec.Name)

	if err := s.StartContainer(spec.Name); err != nil {
		if remErr := s.RemoveContainer(spec.Name); remErr != nil {
			logger.Errorf(ctx, "failed to remove container after unsuccessful start: %s", remErr.Error())
		}
		return nil, errors.Trace(err)
	}
...
func (s *Server) handleAlreadyExistsError(err error, spec ContainerSpec, ephemeral bool) (*Container, error) {
	if IsLXDAlreadyExists(err) {
		...
		if runningErr != nil {
			...
			logger.Debugf(ctx, "waiting for container to be running: %v", runningErr)
			return nil, errors.Trace(err)
		}
...
func (s *Server) RemoveContainers(names []string) error {
	...
	for _, name := range names {
		if err := s.RemoveContainer(name); err != nil {
			...
			logger.Errorf(ctx, "removing container %q: %v", name, err)
		}
	}
	if len(failed) != 0 {
