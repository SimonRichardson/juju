...
func (env *environ) Bootstrap(ctx environs.BootstrapContext, callCtx envcontext.ProviderCallContext, params environs.BootstrapParams) (*environs.BootstrapResult, error) {
	ctx.Infof(ctx, "%s", bootstrapMessage)
...
func (env *environ) MaybeWriteLXDProfile(pName string, put lxdprofile.Profile) error {
	...
	if hasProfile {
		logger.Debugf(ctx, "lxd profile %q already exists, not written again", pName)
		return nil
	}
	logger.Debugf(ctx, "attempting to write lxd profile %q %+v", pName, put)
	...
	logger.Debugf(ctx, "wrote lxd profile %q", pName)
...
func (env *environ) verifyProfile(pName string) error {
	...
	logger.Debugf(ctx, "lxd profile %q: received %+v ", pName, profile.ProfilePut)
	return nil
}
...
func (env *environ) AssignLXDProfiles(instID string, profilesNames []string, profilePosts []lxdprofile.ProfilePost) (current []string, err error) {
	report := func(err error) ([]string, error) {
		...
		if err != nil && err2 != nil {
			logger.Errorf(ctx, "retrieving profile names for %q: %s", instID, err2)
		}
	...
	for _, name := range deleteProfiles {
		if err := server.DeleteProfile(name); err != nil {
			...
			logger.Debugf(ctx, "failed to delete profile %q: %s", name, err)
		}
	}
	return report(nil)
}
...
func (env *environ) DetectHardware() (*instance.HardwareCharacteristics, error) {
	...
	if err != nil {
		logger.Debugf(ctx, "error parsing endpoint as url: %s", err.Error())
		return nil, nil
	}
