...
func (s *systemdServiceManager) WriteSystemdAgent(
	...
	if systemdLinked {
		logger.Infof(ctx, "wrote %s agent, enabled and linked by systemd", serviceName(machineAgent))
	} else {
		logger.Infof(ctx, "wrote %s agent, enabled and linked by symlink", serviceName(machineAgent))
	}
	return nil
}
...
func (s *systemdServiceManager) writeSystemdAgent(agentName, dataDir, systemdMultiUserDir string) (bool, error) {
	...
	if err = svc.WriteService(); err != nil {
		...
		logger.Infof(ctx, "successfully wrote service for %s:", agentName)
	}

	if s.isRunning() {
		logger.Infof(ctx, "wrote %s agent, enabled and linked by systemd", svcName)
		return true, nil
	}
	...
	logger.Infof(ctx, "wrote %s agent, enabled and linked by symlink", svcName)
	return false, nil
}
...
func (s *systemdServiceManager) CreateAgentConf(name string, dataDir string) (_ common.Conf, err error) {
	defer func() {
		if err != nil {
			logger.Infof(ctx, "failed create agent conf for %s: %s", name, err)
		}
	}()
