...
func (r *resourceDownloadLimiter) Acquire(appName string) {
	if r.globalLock != nil {
		start := time.Now()
		r.globalLock <- struct{}{}
		logger.Debugf(ctx, "acquire global resource download lock for %q, took %dms", appName, time.Now().Sub(start)/time.Millisecond)
	}
	...
	start := time.Now()
	lock <- struct{}{}
	logger.Debugf(ctx, "acquire app resource download lock for %q, took %dms", appName, time.Now().Sub(start)/time.Millisecond)
}
