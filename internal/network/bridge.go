...
func (b *etcNetworkInterfacesBridger) Bridge(devices []DeviceToBridge, reconfigureDelay int) error {
	...
	if result != nil {
		logger.Infof(ctx, "bridgescript result=%v", result.Code)
		if result.Code != 0 {
			logger.Errorf(ctx, "bridgescript stdout\n%s\n", result.Stdout)
			logger.Errorf(ctx, "bridgescript stderr\n%s\n", result.Stderr)
			return errors.Errorf("bridgescript failed: %s", string(result.Stderr))
		}
		logger.Tracef(ctx, "bridgescript stdout\n%s\n", result.Stdout)
		logger.Tracef(ctx, "bridgescript stderr\n%s\n", result.Stderr)
	} else {
		logger.Infof(ctx, "bridgescript returned nothing")
	}
