...
func (s *ImportSuite) TestBadBytes(c *gc.C) {
	...
	importer := migration.NewModelImporter(controller, scope, s.modelManagerService, s.controllerConfigService, s.serviceFactoryGetter, configSchemaSource)
...
func (i *fakeImporter) Import(model description.Model, controllerConfig controller.Config, _ state.MachineService, _ state.ApplicationService, _ config.ConfigSchemaSourceGetter) (*state.Model, *state.State, error) {
	i.model = model
