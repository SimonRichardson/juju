...
func (r loggingRequestRecorder) Record(method string, url *url.URL, res *http.Response, rtt time.Duration) {
	if r.logger.IsLevelEnabled(corelogger.TRACE) {
		r.logger.Tracef(ctx, "request (method: %q, host: %q, path: %q, status: %q, duration: %s)", method, url.Host, url.Path, res.Status, rtt)
	}
}
...
func (r loggingRequestRecorder) RecordError(method string, url *url.URL, err error) {
	if r.logger.IsLevelEnabled(corelogger.TRACE) {
		r.logger.Tracef(ctx, "request error (method: %q, host: %q, path: %q, err: %s)", method, url.Host, url.Path, err)
	}
}
...
func (t *apiRequester) Do(req *http.Request) (*http.Response, error) {
	...
	err := retry.Call(retry.CallArgs{
		...
		NotifyFunc: func(lastError error, attempt int) {
			t.logger.Errorf(ctx, "Charmhub API error (attempt %d): %v", attempt, lastError)
		},
		Attempts: 2,
		Delay:    t.retryDelay,
...
func (t *apiRequestLogger) Do(req *http.Request) (*http.Response, error) {
	if t.logger.IsLevelEnabled(corelogger.TRACE) {
		if data, err := httputil.DumpRequest(req, true); err == nil {
			t.logger.Tracef(ctx, "%s request %s", req.Method, data)
		} else {
			t.logger.Tracef(ctx, "%s request DumpRequest error %s", req.Method, err.Error())
		}
	}
	...
	if t.logger.IsLevelEnabled(corelogger.TRACE) {
		if data, err := httputil.DumpResponse(resp, true); err == nil {
			t.logger.Tracef(ctx, "%s response %s", req.Method, data)
		} else {
			t.logger.Tracef(ctx, "%s response DumpResponse error %s", req.Method, err.Error())
		}
	}
