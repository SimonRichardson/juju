...
func (rh *runHook) afterHook(stdCtx stdcontext.Context, state State) (_ bool, err error) {
	defer func() {
		if err != nil {
			rh.logger.Errorf(ctx, "error updating workload status after %v hook: %v", rh.info.Kind, err)
		}
	}()
	...
	case hooks.Start:
		...
		rh.logger.Debugf(ctx, "unit %v has started but has not yet set status", ctx.UnitName())
