...
func (s *PathsSuite) TestOther(c *tc.C) {
	...
	paths := shared.NewPaths(dataDir, unitTag, nil)
	...
	c.Assert(paths, tc.DeepEquals, shared.Paths{
		ToolsDir: relData("tools/unit-some-application-323"),
		Runtime: shared.RuntimePaths{
			LocalJujuExecSocket:    shared.SocketPair{localRunSocket, localRunSocket},
			LocalJujucServerSocket: shared.SocketPair{localJujucSocket, localJujucSocket},
		},
		State: shared.StatePaths{
			BaseDir:         relAgent(),
...
func (s *PathsSuite) TestWorkerPaths(c *tc.C) {
	...
	unitTag := names.NewUnitTag("some-application/323")
	worker := "worker-id"
	paths := shared.NewWorkerPaths(dataDir, unitTag, worker, nil)
...
func (s *PathsSuite) TestContextInterface(c *tc.C) {
	paths := shared.Paths{
		...
		Runtime: shared.RuntimePaths{
			LocalJujucServerSocket: shared.SocketPair{Server: sockets.Socket{Network: "unix", Address: "/path/to/socket"}},
		},
		State: shared.StatePaths{
