...
func admissionHandler(logger logger.Logger, rbacMapper RBACMapper, legacyLabels bool) http.Handler {
	...
	return http.HandlerFunc(func(res http.ResponseWriter, req *http.Request) {
		...
		if err != nil {
			logger.Errorf(ctx, "digesting admission request body: %v", err)
		...
		finalise := func(review *admission.AdmissionReview, response *admission.AdmissionResponse) {
			...
			if err != nil {
				logger.Errorf(ctx, "marshaling admission request response body: %v", err)
				http.Error(res, fmt.Sprintf("%s: building response body",
					http.StatusText(http.StatusInternalServerError)), http.StatusInternalServerError)
			}
			if _, err := res.Write(body); err != nil {
				logger.Errorf(ctx, "writing admission request response body: %v", err)
			}
		}
		...
		logger.Debugf(ctx, "received admission request for %s of %s in namespace %s",
		...
		for _, ignoreObjKind := range admissionObjectIgnores {
			if compareAPIGroupVersionKind(ignoreObjKind, admissionReview.Request.Kind) {
				logger.Debugf(ctx, "ignoring admission request for gvk %s", ignoreObjKind)
