...
func desiredPeerGroup(info *peerGroupInfo) (desiredChanges, error) {
	logger.Debugf(ctx, info.getLogMessage())
...
func (p *peerGroupChanges) possiblePeerGroupChanges() {
	...
	sortAsInts(nodeIds)
	logger.Debugf(ctx, "assessing possible peer group changes:")
	for _, id := range nodeIds {
		...
		if m.host.Life() != state.Alive {
			...
			logger.Debugf(ctx, "controller %v has died %q, wants vote: %v", id, m.host.Life(), m.WantsVote())
		...
		case wantsVote && isVoting:
			logger.Debugf(ctx, "node %q is already voting", id)
			p.toKeepVoting = append(p.toKeepVoting, id)
		case wantsVote && !isVoting:
			if status, ok := p.info.statuses[id]; ok && isReady(status) {
				logger.Debugf(ctx, "node %q is a potential voter", id)
				p.toAddVote = append(p.toAddVote, id)
			} else if member != nil {
				logger.Debugf(ctx, "node %q exists but is not ready (status: %v, healthy: %v)",
					id, status.State, status.Healthy)
				p.toKeepNonVoting = append(p.toKeepNonVoting, id)
			} else {
				logger.Debugf(ctx, "node %q does not exist and is not ready (status: %v, healthy: %v)",
					id, status.State, status.Healthy)
		...
		case !wantsVote && isVoting:
			...
			if isPrimaryMember(p.info, id) {
				p.desired.stepDownPrimary = true
				logger.Debugf(ctx, "primary node %q is a potential non-voter", id)
			} else {
				logger.Debugf(ctx, "node %q is a potential non-voter", id)
			}
		case !wantsVote && !isVoting:
			logger.Debugf(ctx, "node %q does not want the vote", id)
			p.toKeepNonVoting = append(p.toKeepNonVoting, id)
		}
	}
	logger.Debugf(ctx, "assessed")
}
...
func (p *peerGroupChanges) reviewPeerGroupChanges() {
	...
	if keptVoters == 0 {
		...
		for _, id := range p.toRemoveVote {
			...
			if !isPrimary {
				...
				logger.Debugf(ctx, "asked to remove all voters, preserving primary voter %q", id)
	...
	if (newCount)%2 == 1 {
		logger.Debugf(ctx, "number of voters is odd")
		// if this is true we will create an odd number of voters
		return
	}
	if len(p.toAddVote) > 0 {
		last := p.toAddVote[len(p.toAddVote)-1]
		logger.Debugf(ctx, "number of voters would be even, not adding %q to maintain odd", last)
	...
	for i, id := range p.toKeepVoting {
		if !isPrimaryMember(p.info, id) {
			p.toRemoveVote = append(p.toRemoveVote, id)
			logger.Debugf(ctx, "removing vote from %q to maintain odd number of voters", id)
...
func (p *peerGroupChanges) createNonVotingMember() {
	for _, id := range p.toKeepCreateNonVotingMember {
		logger.Debugf(ctx, "create member with id %q", id)
		p.info.maxMemberId++
...
func (p *peerGroupChanges) updateAddressesFromInternal() error {
	...
	for _, id := range ids {
		...
		if len(addrs) == 1 {
			addr := addrs[0]
			logger.Debugf(ctx, "node %q selected address %q by scope from %v", id, addr, hostPorts)
