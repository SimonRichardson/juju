	...
	return func(hello *tls.ClientHelloInfo) (*tls.Certificate, error) {
		for _, getter := range getters {
			...
			if err != nil && !errors.Is(err, errors.NotFound) && !errors.Is(err, errors.NotImplemented) {
				logger.Errorf(ctx, "finding certificate with SNI getter: %v", err)
			}
			if cert != nil {
				return cert, nil
			}
		}
		logger.Warningf(ctx, "unable to find certificate for server name %q", hello.ServerName)
...
func newTLSConfig(
	...
	certLogger := SNIGetterFunc(func(h *tls.ClientHelloInfo) (*tls.Certificate, error) {
		logger.Debugf(ctx, "getting certificate for server name %q", h.ServerName)
