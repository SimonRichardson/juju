...
func NewTxnWatcher(config TxnWatcherConfig) (*TxnWatcher, error) {
	...
	w.tomb.Go(func() error {
		...
		if err != nil {
			w.logger.Infof(ctx, "watcher loop failed: %v", err)
		}
		return err
	})
	return w, nil
...
func (w *TxnWatcher) loop() error {
	w.logger.Tracef(ctx, "loop started")
	...
	for {
		...
		if errorOccurred || first {
			...
			if first {
				defer w.killCursor()
				w.logger.Infof(ctx, "txn watcher started")
				close(w.readyChan)
				first = false
			}
		} else {
		...
		if err == nil {
			if errorOccurred {
				w.logger.Infof(ctx, "txn sync watcher resumed after failure")
			}
			errorOccurred = false
			w.flush()
		} else {
			w.logger.Warningf(ctx, "txn watcher sync error: %v", err)
			...
			w.logger.Warningf(ctx, "txn watcher resume queued")
			errorOccurred = true
...
func (w *TxnWatcher) killCursor() {
	...
	for i := 0; i < 2; i++ {
		...
		w.logger.Warningf(ctx, "failed to kill cursor %d: %s", w.cursorId, err.Error())
	}
	w.cursorId = 0
}
	...
	for i := len(changes) - 1; i >= 0; i-- {
		...
		case "update":
			...
			if revno == 0 && change.TxnNumber == 0 {
				...
				// get skipped.
				w.logger.Criticalf(ctx, "update %s %v without revno %s", change.Ns.Collection, change.DocumentKey.Id, j)
		...
		default:
			...
			w.logger.Warningf(ctx, "received bad change type %s", change.OperationType)
			w.resumeToken = change.Id
			continue
		}
		w.logger.Debugf(ctx, "txn watcher: %s %v #%d", change.Ns.Collection, change.DocumentKey.Id, revno)
...
func (w *TxnWatcher) sync() error {
	w.logger.Tracef(ctx, "txn watcher %p starting sync", w)
