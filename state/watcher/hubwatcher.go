...
func newHubWatcher(hub HubSource, clock Clock, modelUUID string, logger logger.Logger) (*HubWatcher, <-chan struct{}) {
	...
	w.tomb.Go(func() error {
		...
		if err != nil && cause != tomb.ErrDying {
			logger.Infof(ctx, "watcher loop failed: %v", err)
		}
		return cause
	})
	return w, started
...
func (w *HubWatcher) receiveEvent(topic string, data interface{}) {
	...
	case TxnWatcherCollection:
		...
		if !ok {
			w.logger.Warningf(ctx, "incoming event not a Change")
			return
		}
		select {
	...
	default:
		w.logger.Warningf(ctx, "programming error, unknown topic: %q", topic)
	}
}
...
func (w *HubWatcher) loop() error {
	w.logger.Tracef(ctx, "%p loop started", w)
	defer w.logger.Tracef(ctx, "loop finished")
	// idle's channel is initially nil, and is only ever set if idleFunc
	// has a value.
	idle := &time.Timer{}
	if w.idleFunc != nil {
		w.logger.Tracef(ctx, "%p set idle timeout to %s", w, HubWatcherIdleTime)
	...
	for {
		...
		case req := <-w.request:
			w.handle(req)
		case <-idle.C:
			w.logger.Tracef(ctx, "%p notify %s idle", w, w.modelUUID)
...
func (w *HubWatcher) flush() bool {
	...
	w.logger.Tracef(ctx, "%p flushing syncEvents: len(%d) cap(%d)", w, len(w.syncEvents), cap(w.syncEvents))
allevents:
	for i := 0; i < len(w.syncEvents); i++ {
		...
		for e := &w.syncEvents[i]; e.ch != nil; e = &w.syncEvents[i] {
			...
			w.logger.Tracef(ctx, "%p sending syncEvent(%d): e.ch=%v %v", w, i, e.ch, outChange)
			select {
			case <-w.tomb.Dying():
			...
			case inChange := <-w.changes:
				...
				continue
			case e.ch <- outChange:
				w.logger.Tracef(ctx, "%p e.ch=%v has been notified %v", w, e.ch, outChange)
				watchersNotified = true
			case <-deadline:
				w.logger.Criticalf(ctx, "%p programming error, e.ch=%v did not accept %v - missing Unwatch?\nwatch source:\n%s",
	...
	w.logger.Tracef(ctx, "%p syncEvents after flush: len(%d), cap(%d) avg(%.1f)", w, len(w.syncEvents), cap(w.syncEvents), w.averageSyncLen)
	if cap(w.syncEvents) > 100 && float64(cap(w.syncEvents)) > 10.0*w.averageSyncLen {
		w.logger.Debugf(ctx, "syncEvents buffer being reset from peak size %d", cap(w.syncEvents))
		w.syncEvents = nil
	}
...
func (w *HubWatcher) handle(req interface{}) {
	w.logger.Tracef(ctx, "%p got request: %s", w, req)
	w.requestCount++
	switch r := req.(type) {
	case reqWatch:
...
func (w *HubWatcher) queueChange(change Change) {
	w.changeCount++
	w.logger.Tracef(ctx, "%p got change document: %#v", w, change)
	...
	for _, info := range w.watches[watchKey{change.C, nil}] {
		...
		w.logger.Tracef(ctx, "%p adding event for collection %q watch %v, syncEvents: len(%d), cap(%d)", w, change.C, info.ch, len(w.syncEvents), cap(w.syncEvents))
	}
	...
	for i, info := range infos {
		if revno > info.revno || revno < 0 && info.revno >= 0 {
			...
			w.syncEvents = append(w.syncEvents, evt)
			w.syncEventDocCount++
			w.logger.Tracef(ctx, "%p adding event for %v watch %v, syncEvents: len(%d), cap(%d)", w, key, info.ch, len(w.syncEvents), cap(w.syncEvents))
		}
	}
}
