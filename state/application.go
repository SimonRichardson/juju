...
func (a *Application) Destroy(store objectstore.ObjectStore) (err error) {
	...
	defer func() {
		logger.Tracef(ctx, "Application(%s).Destroy() => %v", a.doc.Name, err)
		if err == nil {
	...
	if len(op.Errors) != 0 {
		logger.Warningf(ctx, "operational errors destroying application %v: %v", a.Name(), op.Errors)
	}
	return err
}
...
func (op *DestroyApplicationOperation) Build(attempt int) ([]txn.Op, error) {
	...
	case nil:
		...
		if op.Force {
			logger.Debugf(ctx, "forcing application removal")
			return ops, nil
		}
...
func (op *DestroyApplicationOperation) Done(err error) error {
	if err == nil {
		if err := op.eraseHistory(); err != nil {
			if !op.Force {
				logger.Errorf(ctx, "cannot delete history for application %q: %v", op.app, err)
			}
		...
		if err := op.deleteSecrets(); err != nil {
			logger.Errorf(ctx, "cannot delete secrets for application %q: %v", op.app, err)
		}
		return nil
	}
...
func (op *DestroyApplicationOperation) destroyOps(store objectstore.ObjectStore) ([]txn.Op, error) {
	...
	if len(rels) != op.app.doc.RelationCount {
		...
		logger.Tracef(ctx, "DestroyApplicationOperation(%s).destroyOps mismatched relation count %d != %d",
	...
	if op.app.doc.UnitCount == 0 && op.app.doc.RelationCount == removeCount {
		logger.Tracef(ctx, "DestroyApplicationOperation(%s).destroyOps removing application", op.app.doc.Name)
		...
		if op.app.doc.HasResources && !op.CleanupIgnoringResources {
			if op.Force {
				...
				logger.Debugf(ctx, "scheduling forced application %q cleanup", op.app.doc.Name)
			...
			logger.Debugf(ctx, "advancing application %q to dead, waiting for cluster resources", op.app.doc.Name)
	...
	if op.app.doc.UnitCount > 0 {
		logger.Tracef(ctx, "DestroyApplicationOperation(%s).destroyOps UnitCount == %d, queuing up unitCleanup",
...
func (a *Application) changeCharmOps(
	...
	if oldKey != nil {
		...
		if len(op.Errors) != 0 {
			logger.Errorf(ctx, "could not remove old charm references for %v:%v", oldKey, op.Errors)
		}
	}
	...
	buildTxn := func(attempt int) ([]txn.Op, error) {
		...
		if *a.doc.CharmURL == cfg.Charm.URL() {
			...
			if errors.Is(quotaErr, errors.QuotaLimitExceeded) && cfg.Force {
				logger.Warningf(ctx, "%v; allowing upgrade to proceed as the operator specified --force", quotaErr)
...
func (a *Application) ChangeScale(scaleChange int) (int, error) {
	...
	logger.Tracef(ctx, "ChangeScale DesiredScale %v, scaleChange %v, newScale %v", a.doc.DesiredScale, scaleChange, newScale)
	if newScale < 0 {
...
func (a *Application) ClearResources() error {
	...
	buildTxn := func(attempt int) ([]txn.Op, error) {
		...
		logger.Debugf(ctx, "application %q now has no cluster resources, scheduling cleanup", a.doc.Name)
		cleanupOp := newCleanupOp(
			cleanupApplication,
			a.doc.Name,
			false, // force
...
func (a *Application) LeaderSettings() (map[string]string, error) {
	...
	for escapedKey, interfaceValue := range doc.Settings {
		...
		if value, _ := interfaceValue.(string); value != "" {
			...
			logger.Warningf(ctx, "unexpected leader settings value for %s: %#v", key, interfaceValue)
		}
	}
	return result, nil
}
