...
func (sb *storageBackend) destroyStorageInstance(
	...
	buildTxn := func(attempt int) ([]txn.Op, error) {
		...
		case nil:
			return ops, nil
		default:
			logger.Warningf(ctx, "could not destroy storage instance %v: %v", tag.Id(), err)
			return nil, errors.Trace(err)
		}
	}
...
func removeStorageInstanceOps(si *storageInstance, assert bson.D, force bool) ([]txn.Op, error) {
	...
	if owner != nil {
		...
		if err != nil {
			...
			logger.Warningf(ctx, "could not validate owner for storage instance %v during remove: %v", si.StorageTag().Id(), err)
		}
		ops = append(ops, validateRemoveOps...)
		...
		if err != nil {
			...
			logger.Warningf(ctx, "could not decrement owner count for storage instance %v during remove: %v", si.StorageTag().Id(), err)
		} else {
	...
	if err == nil {
		...
		if err != nil {
			...
			logger.Warningf(ctx, "could not get operations to destroy filesystem %v when removing storage instance %v: %v", filesystem.FilesystemTag().Id(), si.StorageTag().Id(), err)
		}
		...
		logger.Warningf(ctx, "could not get filesystem when removing storage instance %v: %v", si.StorageTag().Id(), err)
	}
	...
	if err == nil {
		...
		if !haveFilesystem {
			...
			if err != nil {
				...
				logger.Warningf(ctx, "could not get operations to destroy volume %v when removing storage instance %v: %v", volume.Tag().Id(), si.StorageTag().Id(), err)
			}
		...
		logger.Warningf(ctx, "could not get volume when removing storage instance %v: %v", si.StorageTag().Id(), err)
	}
	return ops, nil
}
...
func removeStorageAttachmentOps(
	...
	if si.doc.AttachmentCount == 1 {
		if si.doc.Life == Dying {
			...
			if err != nil {
				...
				logger.Warningf(ctx, "could not determine operations for storage instance %v removal: %v", si.StorageTag().Id(), err)
			}
			...
			if err != nil {
				...
				logger.Warningf(ctx, "error validating owner for storage instance %v removal: %v", si.StorageTag().Id(), err)
			}
			ops = append(ops, validateRemoveOps...)
			...
			if err != nil {
				...
				logger.Warningf(ctx, "could not decrease refcount for storage instance %v removal: %v", si.StorageTag().Id(), err)
			}
			ops = append(ops, decrefOp)
		}
	}
	...
	if err != nil {
		...
		logger.Warningf(ctx, "could not determine operations to detach storage attachments for storage instance %v unit %v: %v", si.StorageTag().Id(), s.Unit().Id(), err)
	}
...
func removeStorageInstancesOps(im *storageBackend, owner names.Tag, force bool) ([]txn.Op, error) {
	...
	for _, doc := range docs {
		...
		if err != nil {
			...
			logger.Warningf(ctx, "error determining operations for storage instance %v removal: %v", si.StorageTag().Id(), err)
		}
