...
func (op *LeaveScopeOperation) Build(attempt int) ([]txn.Op, error) {
	rulogger.Tracef(ctx, "%v attempt %d to leave scope", op.Description(), attempt+1)
	if attempt > 0 {
	...
	default:
		if op.Force {
			rulogger.Warningf(ctx, "forcing %v to leave scope despite error %v", op.Description(), err)
			return ops, nil
		}
		return nil, err
	}
...
func (ru *RelationUnit) LeaveScope() error {
	...
	if len(errs) != 0 {
		rulogger.Warningf(ctx, "operational errors leaving scope for unit %q in relation %q: %v", ru.unitName, ru.relation, errs)
	}
	return err
}
...
func (op *LeaveScopeOperation) internalLeaveScope() ([]txn.Op, error) {
	...
	rulogger.Debugf(ctx, "%v leaving scope: unit count %d, life %v", op.Description(), op.ru.relation.doc.UnitCount, op.ru.relation.doc.Life)
	...
	if rulogger.IsLevelEnabled(corelogger.TRACE) {
		rulogger.Tracef(ctx, "leave scope ops for %s: %s", op.Description(), pretty.Sprint(ops))
	}
	return ops, nil
}
