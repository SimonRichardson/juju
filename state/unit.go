...
func (u *Unit) PasswordValid(password string) bool {
	...
	if agentHash == "" {
		logger.Errorf(ctx, "%q invalid password, provided agent hash empty", u.Name())
		return false
	}
	if u.doc.PasswordHash == "" {
		logger.Errorf(ctx, "%q invalid password, doc password hash empty", u.Name())
		return false
	}
	app, err := u.Application()
	if err != nil {
		logger.Errorf(ctx, "%q invalid password, error getting application: %s", u.Name(), err.Error())
		return false
	}
	units, err := app.AllUnits()
	if err != nil {
		logger.Errorf(ctx, "%q invalid password, error getting all units: %s", app.Name(), err.Error())
		return false
	}
	for _, unit := range units {
		if u.Name() != unit.Name() && agentHash == unit.doc.PasswordHash {
			logger.Errorf(ctx, "%q invalid password, provided agent hash matches %q password hash", u.Name(), unit.Name())
		}
	}
	return false
}
...
func (op *UpdateUnitOperation) Build(_ int) ([]txn.Op, error) {
	...
	if containerInfo.ProviderId != "" &&
		...
		logger.Debugf(ctx, "unit %q has provider id %q which changed to %q",
...
func (u *Unit) Destroy(store objectstore.ObjectStore) error {
	...
	if len(errs) != 0 {
		logger.Warningf(ctx, "operational errors destroying unit %v: %v", u.Name(), errs)
	}
	return err
}
...
func (op *DestroyUnitOperation) Build(attempt int) ([]txn.Op, error) {
	...
	default:
		if op.Force {
			logger.Warningf(ctx, "forcing unit destruction for %v despite error %v", op.unit.Name(), err)
			return ops, nil
		}
		return nil, err
	}
...
func (op *DestroyUnitOperation) Done(err error) error {
	...
	if err := op.eraseHistory(); err != nil {
		if !op.Force {
			logger.Errorf(ctx, "cannot delete history for unit %q: %v", op.unit.globalKey(), err)
		}
	...
	if err := op.deleteSecrets(); err != nil {
		logger.Errorf(ctx, "cannot delete secrets for unit %q: %v", op.unit, err)
	}
	return nil
}
...
func (u *Unit) destroyHostOps(a *Application, op *ForcedOperation) (ops []txn.Op, err error) {
	if a.doc.Subordinate {
		...
		unitLogger.Tracef(ctx, "unit %v unassigned", u)
		return nil, nil
	}
...
func (op *RemoveUnitOperation) Build(attempt int) ([]txn.Op, error) {
	...
	default:
		if op.Force {
			logger.Warningf(ctx, "forcing unit removal for %v despite error %v", op.unit.Name(), err)
			return ops, nil
		}
		return nil, err
	}
	...
	buildTxn := func(attempt int) ([]txn.Op, error) {
		...
		if unitCURL != nil {
			...
			if err != nil {
				...
				logger.Errorf(ctx, "could not remove old charm references for %s: %v", unitCURL, err)
			}
			if len(op.Errors) != 0 {
				logger.Errorf(ctx, "could not remove old charm references for %s: %v", unitCURL, op.Errors)
			}
