...
func (op *DestroyRelationOperation) Build(attempt int) ([]txn.Op, error) {
	...
	default:
		if op.Force {
			logger.Warningf(ctx, "force destroying relation %v despite error %v", op.r, err)
			return ops, nil
		}
		return nil, err
	}
...
func (r *Relation) Destroy(_ objectstore.ObjectStore) error {
	...
	if len(errs) != 0 {
		logger.Warningf(ctx, "operational errors removing relation %v: %v", r.Id(), errs)
	}
	return err
}
...
func destroyCrossModelRelationUnitsOps(op *ForcedOperation, remoteApp *RemoteApplication, rel *Relation, onlyForTerminated bool) ([]txn.Op, error) {
	...
	logger.Debugf(ctx, "forcing cleanup of units for %v", remoteApp.Name())
	remoteUnits, err := rel.AllRemoteUnits(remoteApp.Name())
	if op.FatalError(err) {
		return nil, errors.Trace(err)
	} else if err != nil {
		logger.Warningf(ctx, "could not get remote units for %q: %v", remoteApp.Name(), err)
	}
...
func (op *DestroyRelationOperation) internalDestroy() (ops []txn.Op, err error) {
	...
	if op.FatalError(err) {
		...
		if remoteApp.IsConsumerProxy() && remoteApp.doc.RelationCount <= 1 && (op.Force || rel.doc.UnitCount == 0) {
			logger.Debugf(ctx, "removing cross model consumer proxy and last relation %d: %v", op.r.doc.Id, op.r.doc.Key)
...
func (r *Relation) destroyOps(ignoreApplication string, op *ForcedOperation) (ops []txn.Op, isRemove bool, err error) {
	...
	if unitsInScope != r.doc.UnitCount {
		if op.Force {
			logger.Warningf(ctx, "ignoring unit count mismatch on relation %v: expected %d units in scope but got %d", r, r.doc.UnitCount, unitsInScope)
		} else {
	...
	if r.doc.UnitCount == 0 || unitsInScope == 0 {
		...
		if err != nil {
			...
			logger.Warningf(ctx, "ignoring error (%v) while constructing relation %v destroy operations since force is used", err, r)
		}
