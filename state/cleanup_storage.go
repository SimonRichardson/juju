...
func (c *dyingEntityStorageCleaner) cleanupStorage(
	...
	if !c.manual {
		for _, f := range filesystems {
			if err := c.sb.RemoveFilesystem(f.FilesystemTag()); err != nil && !errors.Is(err, errors.NotFound) {
				...
				logger.Warningf(ctx, "could not remove filesystem %v for dying %v: %v", f.FilesystemTag().Id(), c.hostTag, err)
			}
		}
	}
	...
	for _, va := range volumeAttachments {
		if detachable, err := isDetachableVolumeTag(c.sb.mb.db(), va.Volume()); err != nil && !errors.Is(err, errors.NotFound) {
			...
			logger.Warningf(ctx, "could not determine if volume %v for dying %v is detachable: %v", va.Volume().Id(), c.hostTag, err)
		} else if !detachable {
		...
		if err := c.sb.DetachVolume(va.Host(), va.Volume(), c.force); err != nil && !errors.Is(err, errors.NotFound) {
			...
			logger.Warningf(ctx, "could not detach volume %v for dying %v: %v", va.Volume().Id(), c.hostTag, err)
		}
	}
	return nil
}
...
func (c *dyingEntityStorageCleaner) destroyNonDetachableFileSystems() ([]*filesystem, error) {
	...
	for _, f := range filesystems {
		if err := c.sb.DestroyFilesystem(f.FilesystemTag(), c.force); err != nil && !errors.Is(err, errors.NotFound) {
			...
			logger.Warningf(ctx, "could not destroy filesystem %v for %v: %v", f.FilesystemTag().Id(), c.hostTag, err)
		}
	}
...
func (c *dyingEntityStorageCleaner) detachFileSystem(fsa FilesystemAttachment) error {
	...
	if err != nil && !errors.Is(err, errors.NotFound) {
		...
		logger.Warningf(ctx, "could not determine if filesystem %v for %v is detachable: %v", filesystem.Id(), c.hostTag, err)
	}
	if detachable {
		if err := c.sb.DetachFilesystem(fsa.Host(), filesystem); err != nil && !errors.Is(err, errors.NotFound) {
			...
			logger.Warningf(ctx, "could not detach filesystem %v for %v: %v", filesystem.Id(), c.hostTag, err)
		}
	}
	...
	if !detachable {
		...
		if err != nil {
			if !errors.Is(err, errors.NotFound) {
				...
				logger.Warningf(ctx, "could not get filesystem %v for %v: %v", filesystem.Id(), c.hostTag, err)
			}
			return nil
		}
	...
	if err := c.sb.RemoveFilesystemAttachment(fsa.Host(), filesystem, c.force); err != nil && !errors.Is(err, errors.NotFound) {
		...
		logger.Warningf(ctx, "could not remove attachment for filesystem %v for %v: %v", filesystem.Id(), c.hostTag, err)
	}
	if volumeTag != (names.VolumeTag{}) {
		if err := c.sb.RemoveVolumeAttachmentPlan(machineTag, volumeTag, c.force); err != nil && !errors.Is(err, errors.NotFound) {
			...
			logger.Warningf(ctx, "could not remove attachment plan for volume %v for %v: %v", volumeTag.Id(), c.hostTag, err)
		}
	}
	if err := updateStatus(); err != nil && !errors.Is(err, errors.NotFound) {
		...
		logger.Warningf(ctx, "could not update status while cleaning up storage for dying %v: %v", c.hostTag, err)
	}
