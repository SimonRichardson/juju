...
func (c *lock) Acquire(spec Spec) (func(), error) {
	...
	c.mu.Unlock()
	c.logger.Debugf(ctx, "acquire machine lock %q for %s (%s)", mSpec.Name, spec.Worker, spec.Comment)
	...
	c.logger.Debugf(ctx, "machine lock %q acquired for %s (%s)", mSpec.Name, spec.Worker, spec.Comment)
	c.holder = current
	...
	return func() {
		...
		c.writeLogEntry()
		c.logger.Debugf(ctx, "machine lock %q released for %s (%s)", mSpec.Name, spec.Worker, spec.Comment)
		releaser.Release()
...
func (c *lock) writeLogEntry() {
	...
	c.logger.Debugf(ctx, "created rotating log file %q with max size %d MB and max backups %d",
	...
	if c.startMessage != "" {
		...
		if err != nil {
			c.logger.Warningf(ctx, "unable to write startMessage: %s", err.Error())
		}
		c.startMessage = ""
	}
	...
	if err != nil {
		c.logger.Warningf(ctx, "unable to release message: %s", err.Error())
	}
}
