...
func GetObservedNetworkConfig(source ConfigSource) (InterfaceInfos, error) {
	logger.Tracef(ctx, "discovering observed machine network config...")
	...
	if len(interfaces) == 0 {
		logger.Tracef(ctx, "no network interfaces")
		return nil, nil
	}
	...
	if len(noAddressesNics) > 0 {
		logger.Debugf(ctx, "no addresses observed on interfaces %q", strings.Join(noAddressesNics, ", "))
	}
...
func addressesToConfig(nic InterfaceInfo, nicAddrs []ConfigSourceAddr) ([]ProviderAddress, error) {
	...
	for _, nicAddr := range nicAddrs {
		...
		if ip.To4() == nil && ip.IsLinkLocalUnicast() {
			logger.Tracef(ctx, "skipping observed IPv6 link-local address %q on %q", ip, nic.InterfaceName)
			continue
		}
