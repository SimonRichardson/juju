...
// TestingSSHServerListener returns a listener and a closedAllowed channel. You
// are expected to receive from the closeAllowed channel within your Close()
// function. The channel is closed once an accept has occurred at least once.
func TestingSSHServerListener(l net.Listener, timeout time.Duration) net.Listener {
...
func (s *listenerSuite) TestAcceptOnceListener(c *gc.C) {
	...
	acceptOnceListener := TestingSSHServerListener(s.listener, time.Second)
...
func (s *listenerSuite) TestAcceptOnceListenerDoesNotStop(c *gc.C) {
	...
	acceptOnceListener := TestingSSHServerListener(s.listener, time.Millisecond*50)
