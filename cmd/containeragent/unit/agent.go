...
func (c *containerUnitAgent) Run(ctx *cmd.Context) (err error) {
	defer c.Done(err)
	ctx.Infof(ctx, "starting containeragent unit command")
	...
	ctx.Infof(ctx, "containeragent unit %q start (%s [%s])", c.Tag().String(), jujuversion.Current, runtime.Compiler)
	if flags := featureflag.String(); flags != "" {
		ctx.Warningf(ctx, "developer feature flags enabled: %s", flags)
	}
