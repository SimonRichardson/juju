...
func recursiveTar(srcBase, srcFile, destBase, destFile string, tw *tar.Writer) error {
	...
	for _, fpath := range matchedPaths {
		...
		if stat.IsDir() {
			...
			logger.Warningf(ctx, "socket file %q ignored", fpath)
		} else {
...
func unTarAll(src FileResource, reader io.Reader, destDir, prefix string) error {
	...
	for {
		...
		if !isDestRelative(destDir, destFileName) {
			logger.Warningf(ctx, "file %q is outside target destination, skipping", destFileName)
			continue
		}
		...
		if mode&os.ModeSymlink != 0 {
			logger.Warningf(ctx, "skipping symlink: %q -> %q", destFileName, header.Linkname)
			continue
		}
