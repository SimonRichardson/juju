...
func (a *app) Ensure(config caas.ApplicationConfig) (err error) {
	...
	defer func() {
		if err != nil {
			logger.Errorf(ctx, "Ensure %s", err)
		}
	}()
	logger.Debugf(ctx, "creating/updating %s application", a.name)
...
func (a *app) Exists() (caas.DeploymentState, error) {
	...
	for _, c := range checks {
		...
		if terminating || c.forceTerminating {
			...
			logger.Debugf(ctx, "application %q exists and is terminating due to dangling %s resource(s)", a.name, c.label)
